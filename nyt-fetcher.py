...
response = requests.get('https://api.nytimes.com/svc/books/v3/lists/current/combined-print-and-e-book-fiction.json?api-key=HZt6QgrKva8Vbj4BOGkxgIRkGmTz2e2H').text
...
response = requests.get('https://api.nytimes.com/svc/books/v3/lists/current/combined-print-and-e-book-nonfiction.json?api-key=HZt6QgrKva8Vbj4BOGkxgIRkGmTz2e2H').text
re = json.loads(response, object_hook=lambda d: SimpleNamespace(**d))
print(re.results.published_date)
books = re.results.books
list = []
